...
class Comb:
	...
	def copyVideo(seld, public_id, count, source):
		...
		if type(source)==int:
			...
			while step<1000:
				step+=200
				for i in vkapi.video.get(owner_id=public_id, offset=step, count=200)['items']:
					if updateDate!=None:
						if updateDate==i['date']:
							break
					print(i['title'])
		...
		if source=='search':
			...
			for i in users:
				if i['id'] not in [i for i in vkapi.friends.get(owner_id=person[0])['items']] and vkapi.friends.areFriends(user_ids=i['id'], need_sign=1)[0]['friend_status']!=1:
					...
					add= requests.get('https://api.vk.com/method/friends.add?user_id='+str(i['id'])+'&text=Привет, '+i['fn']+'&access_token='+accTok).json()
					time.sleep(1)
					if 'error' in add:
						print(add['error'])
						captchaSid=add['error']['captcha_sid']
						captchaKey=Comb.captcha('self', add['error']['captcha_img'])
						add=requests.get('https://api.vk.com/method/friends.add?user_id='+str(i['id'])+'&text=Привет, '+i['fn']+'.&captcha_sid='+str(captchaSid)+'&captcha_key='+str(captchaKey)+'&access_token='+accTok).json()
					time.sleep(1)
					# users.append(b['id'])
...
if __name__ == "__main__":
	...
	def actions():
		...
		# 	requests.get("https://api.vk.com/method/photos.edit?owner_id="+str(person[0])+"&photo_id=377608717&caption=sexysexey&v=5.37&access_token="+accTok).json()
			...
			while step < 500:
				step+=80
			for i in vkapi.wall.get(owner_id=public, count=15)['items']:
