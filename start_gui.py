...
class Comb:
	...
	def copyPhotoToAlbum( self, to_id, from_id, albumNameToCopyTo, albumIdToCopyFrom, countPhotos, text=False):
		...
		if countPhotos>1000:
			...
			if albumIdToCopyFrom=='wall':
				Comb.UpdateData('self', from_id, photosGet['items'][0]['date'], 'photo', 'wall')
			elif albumIdToCopyFrom!='wall':
				Comb.UpdateData('self', from_id, photosGet['items'][0]['date'], 'photo', 'album', albumIdToCopyFrom )
	...
	def copyVideo(seld, public_id, count, source):
		...
		if source=='wall' or source=='videobox':
			...
			for i in vkapi.video.getAlbums(owner_id=person[0], extended=1, count=100)['items']:
				...
				if re.sub('[0-9]', '', publicName) in re.sub('[0-9]', '', i['title']):
						# print(i['title'])
						# TargetAlbumId=i['id']
	...
	def audioStat(self):
		...
		# while step<400:
		# 	step+=100
		# 	for i in vkapi.audio.getAlbums(owner_id=person[0], count=100, offset=step)['items']:
		# 		total_albums+=1
		step=-6000
		# print('Total audio albums: %s' % (total_albums))
		time.sleep(1)
		# total_audios=vkapi.audio.get(owner_id=person[0], count=6000, offset=step)['count']
		while step<20000:
			step+=6000
			for i in vkapi.audio.get(owner_id=person[0], count=6000, offset=step, need_user=0)['items']:
				total_audios+=1
			
		print('Total audio albums: %s\nTotal audios: %s' % (total_albums, total_audios))
